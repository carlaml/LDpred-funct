...
- --flips: use the sign from the raw effect, which is basically the one form the coordinate file
...
def get_prs(genotype_file, rs_id_map, phen_map=None,lasso=False,sets=False):
...
#         n_snp = (snp - sp.mean(snp))/sp.std(snp)
        ...
        if lasso:
            pval_derived_effects_prs_lasso += snp*upd_pval_beta_lasso
            assert not sp.any(sp.isnan(pval_derived_effects_prs_lasso)),'Weighted effects PRS is corrupted'
            if sets:
                pval_derived_effects_prs_high += snp * upd_pval_beta_high
                assert not sp.any(sp.isnan(pval_derived_effects_prs_high)), 'Weighted effects PRS is corrupted'
                pval_derived_effects_prs_lasso_high += snp * upd_pval_beta_lasso_high
                assert not sp.any(sp.isnan(pval_derived_effects_prs_lasso_high)), 'Weighted effects PRS is corrupted'

                pval_derived_effects_prs_low += snp * upd_pval_beta_low
                assert not sp.any(sp.isnan(pval_derived_effects_prs_low)), 'Weighted effects PRS is corrupted'
                pval_derived_effects_prs_lasso_low += snp * upd_pval_beta_lasso_low
                assert not sp.any(sp.isnan(pval_derived_effects_prs_lasso_low)), 'Weighted effects PRS is corrupted'
    ...
    if lasso:
        ret_dict = {'raw_effects_prs': raw_effects_prs.copy(),
                    'pval_derived_effects_prs': pval_derived_effects_prs.copy(),
                    'pval_derived_effects_prs_lasso': pval_derived_effects_prs_lasso.copy(),
                    'true_phens': true_phens[:], 'iids': iids}
        if sets:
            ret_dict = {'raw_effects_prs': raw_effects_prs.copy(),
                        'pval_derived_effects_prs': pval_derived_effects_prs.copy(),
                        'pval_derived_effects_prs_lasso': pval_derived_effects_prs_lasso.copy(),
                        'pval_derived_effects_prs_high': pval_derived_effects_prs_high.copy(),
                        'pval_derived_effects_prs_lasso_high': pval_derived_effects_prs_lasso_high.copy(),
                        'pval_derived_effects_prs_low': pval_derived_effects_prs_low.copy(),
                        'pval_derived_effects_prs_lasso_low': pval_derived_effects_prs_lasso_low.copy(),
                        'true_phens': true_phens[:], 'iids': iids}
...
def calc_risk_scores(bimfile_name, rs_id_map, phen_map, K_bins=1,out_file=None):
    ...
    if bimfile_name is not None:
        ...
        for bimfile in bimfile_list:
            ...
            prs_dict = get_prs_bins(genotype_file, rs_id_map, phen_map=phen_map,K_bins=K_bins)
